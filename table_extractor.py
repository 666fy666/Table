...
def rec_table(file_path, output, pre=False, logic_info=False, box=False):
    # # 判断文件格式
    # if file_path.endswith(('.jpg', '.png', '.jpeg')):
    #     img_path = file_path
    # elif file_path.endswith('.pdf'):
    #     images = convert_from_path(file_path, 300)
    #     img_path = f"{file_path.split('.')[0]}.png"
    ...
    if file_path.endswith(('.jpg', '.png', '.jpeg', '.tiff', '.tif', '.bmp')):
        img_paths = [file_path]
    elif file_path.endswith('.pdf'):
        images = convert_from_path(file_path, 300)
        img_paths = [f"{file_path.split('.')[0]}_{i}.png" for i in range(len(images))]
    ...
    for idx, img_path in enumerate(img_paths):
        ...
        complete_htmls.append(complete_html)
    ...
    # html转excel
    excel_file = html_table_to_excel(complete_htmls, f"{output}/{file_name}.xlsx")
    print(excel_file)
    if logic_info:
        # 可视化表格识别框 + 逻辑行列信息
...
def html_table_to_excel(htmls, excel_path):
    # 初始化Excel
...
if __name__ == '__main__':
    # 测试单张图片
    result_excel = rec_table(file_path="test.pdf",output="./")
    print("the result excel is:", result_excel)
